...
function getSavedRowsForDate_(dateKey) {
  const cache = loadLastCallCache_();
  const cacheRows = cache && String(cache.dateKey || '') === String(dateKey || '') ? (cache.rows || []) : [];
  ...
  return mergePresenceRows_(
    mergePresenceRows_(baseRows, cacheRows),
    selfRows
  );
}
