...
## Decisão
A edição de aluno continua acontecendo em uma página própria em `aluno/editar-aluno/`, aberta a partir do botão **Editar**. O identificador de edição prioriza o nome atual do aluno, que é a chave usada pelo fluxo manual confirmado no backend. O código exibido após `#` permanece visível apenas na tela de edição e não aparece mais nos cartões da lista.
