...
## Solução
Garantir que a sessão autenticada carregue `idCadastro`/`id_cadastro` e que a camada central de requisições envie `x-cadastro-id` nas chamadas autenticadas. Durante a migração, `GET /api/classes` também pode receber `id_cadastro` por compatibilidade.
