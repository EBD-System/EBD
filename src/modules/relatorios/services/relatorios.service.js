(function initRelatoriosService(root) {
  ...
  const service = {
    ...
    async fetchPeriodPdfDetails({ startDate, endDate, token } = {}) {
      if (!isIsoDate(startDate) || !isIsoDate(endDate)) {
        return {
          found: false,
          reason: 'Informe uma data inicial e uma data final válidas.'
      ...
      const url = `${REPORT_PDF_ENDPOINT}?startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`;
      const response = await fetch(url, {
        headers: {
          Authorization: `Bearer ${token}`
        }
